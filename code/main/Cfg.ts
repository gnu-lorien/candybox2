...
class Cfg extends Place{
    ...
    private drawAbout(x: number, y: number): void{
        ...
        // Contact
        this.renderArea.drawString("Feel free to contact me at aniwey@gmail.com if you have any comment or question :)", x + 4, y + 24);
        
        // By the way
        this.renderArea.drawString("(by the way, I'm looking for a ten weeks work placement starting around June to complete my studies)", x, y + 27);
        this.renderArea.drawString("(if any company creating cool games is interested, feel free to contact me!)", x + 12, y + 28);
    }
    ...
    private drawCfgLanguage(x: number, y: number): void{
        ...
        this.renderArea.addList(x + Algo.takeBiggest(Database.getText("cfgChooseLanguage").length, Database.getTranslatedText("cfgChooseLanguage").length) + 2, x + Algo.takeBiggest(Database.getText("cfgChooseLanguage").length, Database.getTranslatedText("cfgChooseLanguage").length) + 20, y, "cfgLanguageList", new CallbackCollection(this.languageSelected.bind(this)),
            ...
            "cfgLanguageNl", "Dutch (by Noël Wierema and Vincent van Gennep)",
            "cfgLanguageFr", "French"
            ]
        );
